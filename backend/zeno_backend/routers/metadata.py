...
async def get_models(project_uuid: str, request: Request):
    ...
    return await select.models(project_uuid)
...
async def get_columns(project_uuid: str, request: Request):
    ...
    """
    await util.project_access_valid(project_uuid, request)
    return await select.columns(project_uuid)
...
async def get_home_details(home_request: HomeRequest, req: Request):
    ...
    """
    user = await util.get_user_from_token(req)
    ...
    if (
        ...
        if user and home_request.user_name is not None:
            projects = await select.projects(user, home_request)
        else:
            projects = await select.public_projects(home_request)
    project_ret: list[HomeEntry] = []
    for proj in projects:
        stats = await select.project_stats(proj.uuid, user.id if user else None)
    ...
    if (
        ...
        if user and home_request.user_name is not None:
            reports = await select.reports(user, home_request)
        else:
            reports = await select.public_reports(home_request)
    report_ret: list[HomeEntry] = []
    for rep in reports:
        stats = await select.report_stats(rep.id, user.id if user else None)
