...
export async function getPointOpacities(
	...
	const filteredIds = (await getFilteredIds(selectionPredicates, get(model), tagIds)) as string[];
	const filteredIdsIndex = new Map(filteredIds.map((id, index) => [id, index]));
	if (filteredIds.length > 0) {
		return points.ids.map((id) => (filteredIdsIndex.has(id) ? fullOpacity : partialOpacity));
	} else {
